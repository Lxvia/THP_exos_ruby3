...
def how_many_handle
    puts "======================"
    puts "There is #{twitter_accounts.count} handle in this array"
end


def shortest_handle
    puts "======================"
    puts "The shortest handle of the list is #{twitter_accounts.min_by(&:length)}"
end

def handle_5
    puts "======================"
    puts "Those handles are 5 characters long :"
    twitter_accounts.each do |tweetname|
        if tweetname.length == 6
            puts "#{tweetname}"
        end
    end
end
...
def handle_maj
    puts "======================"
    tweetname = twitter_accounts.select {|result| result[0..1] =~ /[A-Z]/}.count
    puts "There are #{tweetname} handles that starts with uppercase"
end

def handle_abc
    puts "======================"
    puts "#{twitter_accounts.sort{ |a,b| a<=>b}}"
end

def handle_smaller_first
    puts "======================"
    puts "#{twitter_accounts.sort_by{ |tweetname| tweetname.size}}"
end

def epenser_position
    puts "======================"
     puts " '@epenser' is the number #{twitter_accounts.index("@epenser")} in this array"
end
